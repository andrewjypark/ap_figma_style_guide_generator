...
function computeConsumedSiblingIds(vars, siblingIndex) {
  ...
  for (const v of vars) {
    if (v.resolvedType !== "FLOAT" && v.resolvedType !== "STRING") continue;
    if (inferTokenKind(v.name) !== "font-size") continue;
...
// not a numeric weight, so it binds via Figma's "fontStyle" field) —
// and returns a compact, human-readable readout of what actually got
// applied, in render order, for the card's documentation line. Text-case
// has no bindable variable field in Figma, so it's applied literally to
// the sample and flagged "(unbound)" in the readout rather than silently
// looking like a real binding.
async function composeStyleSample(sample, sizeVar, siblings, modeId) {
  ...
  if (siblings.familyVar) {
    try {
      ...
      if (typeof resolved === "string") family = resolved;
    } catch (e) {}
  }
  if (siblings.weightVar) {
    try {
      const resolved = await resolveVariableValue(siblings.weightVar, modeId);
      if (typeof resolved === "string") weight = resolved;
    } catch (e) {}
  }
  if (family || weight) {
    const current = sample.fontName;
    const candidate = { family: family || current.family, style: weight || current.style };
    const loadedFont = await tryLoadFont(candidate);
    if (loadedFont) {
      sample.fontName = loadedFont;
      if (family) tryBindVariable(sample, "fontFamily", siblings.familyVar);
      if (weight) tryBindVariable(sample, "fontStyle", siblings.weightVar);
      readout.push(loadedFont.family + (weight ? " " + loadedFont.style : ""));
    }
  }
  ...
  if (siblings.lineHeightVar) {
    try {
      ...
      if (typeof resolved === "number") {
        ...
        readout.push(resolved + "px line-height");
      }
    } catch (e) {}
  }
  ...
  if (siblings.letterSpacingVar) {
    try {
      ...
      if (typeof resolved === "number") {
        ...
        readout.push(resolved + "px letter-spacing");
      }
    } catch (e) {}
  }
    ...
    try {
      ...
      if (typeof resolved === "string") {
        ...
        if (textCase) {
          ...
          readout.push(resolved.toLowerCase() + " (unbound)");
        }
      }
    } catch (e) {}
  }
...
async function buildTokenCard(v, labelParts, modeId, siblingIndex) {
  ...
  if (resolved !== null) {
    try {
      if (kind === "line-height" && typeof resolved === "number") {
        sample.lineHeight = { value: resolved, unit: "PIXELS" };
        applied = true;
...
// literally), then returns a compact "56px · IBM Plex Sans · 67px
// line-height · ..." readout of what actually got applied, so the card
// still documents the underlying values even though they're shown fused
// into one rendered style rather than as separate property call-outs.
async function buildComposedTokenCard(sizeVar, groupLabelParts, modeId, siblingIndex) {
  ...
  try {
    ...
    readout = await composeStyleSample(sample, sizeVar, siblings, modeId);
  } catch (e) {
    // composition failed; the sample keeps its plain default styling.
  }

  if (readout.length) {
    const readoutText = await makeText(readout.join("   ·   "), FONTS.mono, 12, PALETTE.grey, {
      lineHeight: { unit: "PIXELS", value: 16 },
      letterSpacing: { unit: "PIXELS", value: 0.5 },
    });
    card.appendChild(readoutText);
  }
...
async function buildTokenGroupSection(group, modeId, siblingIndex) {
  ...
  for (let i = 0; i < group.vars.length; i += TOKEN_CARDS_PER_ROW) {
    ...
    for (const v of chunk) {
      const card =
        inferTokenKind(v.name) === "font-size"
...
async function buildOutputSection(sectionLabel, headerTitle, headerDescription, groupSections, xOffset, outputPage, collection, modeId, modeName) {
  ...
  wrapper.x = MARGIN;
  wrapper.y = MARGIN;
  section.resizeWithoutConstraints(wrapper.width + MARGIN * 2, wrapper.height + MARGIN * 2);
  section.x = xOffset;
  section.y = 0;
